 ...
 * Phi.ts — MD5 哈希工具
 *
 * 用于计算谱面字符串的 MD5 哈希，作为成绩存储的谱面 ID。
 * 与原版 phigros-html5 使用的 md5-js 库输出格式一致（32 字符小写十六进制）。
 *
 * 实现: 基于 RFC 1321 的紧凑型纯 JS 实现，支持 UTF-8 编码输入。
 *
...
function bytesToWords(bytes: number[]): number[] {
  ...
  for (let i = 0; i < bytes.length * 8; i += 8) {
    out[i >> 5] = (out[i >> 5] || 0) | (bytes[i / 8] << (i % 32));
  }
  return out;
}
...
/**
 * 计算字符串的 MD5 哈希（32 字符小写十六进制）
 *
 * @param input 输入字符串（UTF-8 编码）
 * @returns 32 字符十六进制字符串
 */
export function md5(input: string): string {
  const bytes = strToBytes(input);
  const words = bytesToWords(bytes);
  const result = core(words, bytes.length * 8);
